...
void eval(char *cmdline) {
    ...
    sigset_t mask_all, mask_one, prev_one;  // Shield signal
    ...
    if (!builtin_cmd(argv)) {
        // Block all signals, make sure these signals are not handled with
        // default disposition before they are read
        sigfillset(&mask_all);
        sigemptyset(&mask_one);
        sigaddset(&mask_one, SIGCHLD); // Ignore SIGCHLD before fork()
        sigprocmask(SIG_BLOCK, &mask_one, &prev_one);

        if ((pid = fork()) == 0) {
            // Distinguished from the process group of the parent process
            setpgid(0, 0);
