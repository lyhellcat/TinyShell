// https://github.com/hungys/mysh/blob/cb2474002a9f1b2a09b61abce386c273e414f772/mysh.c
// https://github.com/yzhh029/ece695/blob/master/myshell/myshell.c
#include <fcntl.h>
#include <glob.h>
#include <pwd.h>
#include <signal.h>
...
#define COLOR_GRAY "\033[1;30m"
       ...
       const char *STATUS_STRING[] = {"running", "done", "suspended",
...
int print_job_status(int id) {
    ...
    struct process *proc;
    for (proc = shell->jobs[id]->root; proc != NULL; proc = proc->next) {
        printf("\t%d\t%s\t%s", proc->pid, STATUS_STRING[proc->status],
               proc->command);
        if (proc->next != NULL) {
            printf("|\n");
        } else {
...
void mysh_update_cwd_info() { getcwd(shell->cur_dir, sizeof(shell->cur_dir)); }
...
int mysh_launch_process(struct job *job, struct process *proc, int in_fd,
                        int out_fd, int mode) {
    ...
    for (; i < position; i++) {
        if (tokens[i][0] == '<') {
            if (strlen(tokens[i]) == 1) {
                input_path =
                    (char *)malloc((strlen(tokens[i + 1]) + 1) * sizeof(char));
...
void mysh_print_welcome() { printf("Welcome to mysh by 0456018!\n"); }
